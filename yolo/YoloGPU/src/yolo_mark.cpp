...
int yolo_resize(int argc, char** argv)
{
...
int cutpicforvideo(int argc, char** argv)
{
    //PhotoCV 1.mp4 100
    VideoCapture vcap;
    cv::Mat fps_curr;
    ...
    if (!vcap.isOpened())
        return 1;
    int frameH = vcap.get(CV_CAP_PROP_FRAME_HEIGHT);
    int frameW = vcap.get(CV_CAP_PROP_FRAME_WIDTH);
    int fps = vcap.get(CV_CAP_PROP_FPS);
    int numFrames = vcap.get(CV_CAP_PROP_FRAME_COUNT);
    printf("width=%d height=%d fps=%d nums=%05d\n", frameW, frameH, fps, numFrames);
    int i(1);
    int k(0);
    while (true)
    {
        ...
        {
            k = 0;
            sprintf(name, "%s/%04d.jpg", argv[3], i++);
            printf("%s\n", name);
            imwrite(name, fps_curr);
        }
        waitKey(1);
    }
    return 0;
...
int yolo_mark(int argc, char** argv)
{
    try
    {
        ...
        if (argc >= 3)
        {
            images_path = std::string(argv[2]);         // path to images, train and synset
        }
        else
        {
        ...
        if (argc >= 5 && (train_filename == "cap_video" || train_filename == "cap_video_backward"))
        {
            ...
            if (argc >= 6)
                save_each_frames = std::stoul(std::string(argv[5]));
            int pos_filename = 0;
        ...
        std::string const window_name = "素材标记工具";
        ...
        std::string const trackbar_name = "素材图片ID";
        //创建图片工具栏
        ...
        std::string const trackbar_name_2 = "标记类别ID";
        ...
        do
        {
            ...
            for (size_t k = 0; k < current_coord_vec.size(); ++k)
            {
                ...
                //Scalar color_rect(red, green, blue);
                //上面五彩缤纷的 还是用下面强制绿色鲜明一些
                Scalar color_rect(0, 255, 0);
                // selected rect
